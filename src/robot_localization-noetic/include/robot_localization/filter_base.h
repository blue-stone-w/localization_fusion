...
class FilterBase
{
 public:
  ...
  //! If the pointer is invalid, the filter will not enter debug mode. If debug is false, outStream is ignored.
  //!
 ...
 protected:
  ...
  //! all we did was make predictions without correcting, the error in our position estimate would grow without
  //! bound. This error is stored in the stateEstimateCovariance_ matrix. However, this matrix doesn't answer the
  //! question of *how much* our error should grow for each time step. That's where the processNoiseCovariance matrix
  //! comes in. When we make a prediction using the transfer function, we add this matrix (times deltaT) to the state
