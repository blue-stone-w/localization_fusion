  ...
  tf_listener_(tf_buffer_)
{
  ...
  {
    ROS_FATAL_STREAM("Process noise covariance not found in the robot localization listener config (namespace " << nh_param.getNamespace( ) << ")! Use the ~parameter_namespace to specify the parameter namespace.");
  }
  else
  {
    try
    ...
    {
      ROS_ERROR_STREAM("ERROR reading robot localization listener config: " << e.getMessage( ) << " for process_noise_covariance (type: " << process_noise_covar_config.getType( ) << ")");
    }
  }
  ...
  ROS_INFO_STREAM("Ros Robot Localization Listener: Listening to topics " << odom_sub_.getTopic( ) << " and " << accel_sub_.getTopic( ));

  // Wait until the base and world frames are set by the incoming messages
  while (ros::ok( ) && base_frame_id_.empty( ))
  {
    ros::spinOnce( );
    ROS_INFO_STREAM_THROTTLE(1.0, "Ros Robot Localization Listener: Waiting for incoming messages on topics " << odom_sub_.getTopic( ) << " and " << accel_sub_.getTopic( ));
                                            ...
                                            std::string world_frame_id) const
{
  ...
  else
  {
    ...
    try
    {
      ...
      {
        ROS_ERROR_STREAM("You are trying to get the state with respect to world frame " << world_frame_id << ", but this frame is a child of robot base frame " << base_frame_id_ << ", so this doesn't make sense.");
        return false;
      }
    }
  ...
  covariance.block<POSE_SIZE, POSE_SIZE>(POSITION_OFFSET, POSITION_OFFSET) =
      rot_6d * estimator_state.covariance.block<POSE_SIZE, POSE_SIZE>(POSITION_OFFSET, POSITION_OFFSET) * rot_6d.transpose( );
