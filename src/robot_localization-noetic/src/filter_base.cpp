...
void FilterBase::reset( )
{
  ...
  // Prepare the invariant parts of the transfer function and variant parts will be updated in subclass
  transferFunction_.setIdentity( );

  // Clear the Jacobian
  transferFunctionJacobian_.setZero( );

  // Set the estimate error covariance. We want our measurements to be accepted rapidly when the filter starts, so we
  // should initialize the state's covariance with large values.
  ...
  // Set the epsilon matrix to be a matrix with small values on the diagonal It is used to maintain the
  // positive-definite property of the covariance
  ...
  // These can be overridden via the launch parameters, but we need default values.
...
void FilterBase::processMeasurement(const Measurement &measurement)
{
  ...
  // If we've had a previous reading, then go through the predict/update cycle. Otherwise, set our state and covariance
  // to whatever we get from this measurement.
  if (initialized_)
  {
    ...
    // Only want to carry out a prediction if it's forward in time. Otherwise, just correct.
    if (delta > 0)
    {
      validateDelta(delta);
  ...
  else
  {
    ...
    for (size_t i = 0; i < measurementLength; ++i)
    {
      ...
      {
        estimateErrorCovariance_(i, j) = (measurement.updateVector_[i] && measurement.updateVector_[j] ? measurement.covariance_(i, j) : estimateErrorCovariance_(i, j));
      }
    }
...
void FilterBase::setControlParams(const std::vector<int> &updateVector,
...
void FilterBase::prepareControl(const double referenceTime, const double predictionDelta)
{
  ...
  if (useControl_)
  {
    ...
    if (timedOut)
    {
      FB_DEBUG("Control timed out. "
               << "Reference time was " << referenceTime
               << ", latest control time was " << latestControlTime_
               << ", control timeout was " << controlTimeout_ << "\n");
    }
                                           ...
                                           const double nsigmas)
{
  ...
  if (sqMahalanobis >= threshold)
  {
    FB_DEBUG("Innovation mahalanobis distance test failed. "
             << "Squared Mahalanobis is: " << sqMahalanobis << "\n"
