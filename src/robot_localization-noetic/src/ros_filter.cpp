                                        ...
                                        const std::string &targetFrame)
{
  ...
  else
  {
    ...
    stream << "The " << topicName << " message has a timestamp before that of the previous message received,"
           << " this message will be ignored. This may indicate a bad timestamp. (message time: " << msg->header.stamp.toSec( ) << ")";
                               ...
                               const CallbackData &accelCallbackData)
{
  ...
  if (msg->header.stamp <= lastSetPoseTime_)
  {
    ...
    stream << "The " << topicName << " message has a timestamp equal to or before the last filter reset, "
           << "this message will be ignored. This may indicate an empty or bad timestamp. (message time: " << msg->header.stamp.toSec( ) << ")";
    addDiagnostic(diagnostic_msgs::DiagnosticStatus::WARN,
                  topicName + "_timestamp",
                  stream.str( ),
                  false);
  ...
  if (!measurementQueue_.empty( ))
  {
    ...
    while (!measurementQueue_.empty( ) && ros::ok( ))
    {
      ...
      if (smoothLaggedData_)
      {
        ...
        if (measurementQueue_.empty( ) || ::fabs(measurementQueue_.top( )->time_ - filter_.getLastMeasurementTime( )) > 1e-9)
        {
...
void RosFilter<T>::loadParams( )
{
  ...
  ROS_FATAL_COND(mapFrameId_ == odomFrameId_ || odomFrameId_ == baseLinkFrameId_ || mapFrameId_ == baseLinkFrameId_ || odomFrameId_ == baseLinkOutputFrameId_ || mapFrameId_ == baseLinkOutputFrameId_,
    ...
    if (moreParams)
    {
      ...
      if (relative && differential)
      {
        ROS_WARN_STREAM("Both " << odomTopicName << "_differential"
                                << " and " << odomTopicName << "_relative were set to true. Using differential mode.");
    ...
    if (moreParams)
    {
      ...
      if (relative && differential)
      {
        ROS_WARN_STREAM("Both " << poseTopicName << "_differential"
                                << " and " << poseTopicName << "_relative were set to true. Using differential mode.");
    ...
    if (moreParams)
    {
      ...
      if (relative && differential)
      {
        ROS_WARN_STREAM("Both " << imuTopicName << "_differential"
                                << " and " << imuTopicName << "_relative were set to true. Using differential mode.");
  ...
  if (getFilteredOdometryMessage(filteredPosition))
  {
    ...
    if (publishTransform_ && !corrected_data)
    {
      ...
      else if (filteredPosition.header.frame_id == mapFrameId_)
      {
        ...
        {
          ROS_ERROR_STREAM_DELAYED_THROTTLE(5.0, "Could not obtain transform from " << odomFrameId_ << "->" << baseLinkFrameId_);
        }
      }
      else
      {
                                       ...
                                       Eigen::MatrixXd &measurementCovariance)
{
  ...
  if (canTransform)
  {
    ...
    tf2::Vector3 stateTwistRot(state(StateMemberVroll),
                               state(StateMemberVpitch),
                               state(StateMemberVyaw));
    accTmp  = targetFrameTrans.getBasis( ) * accTmp + targetFrameTrans.getOrigin( ).cross(angular_acceleration_) - targetFrameTrans.getOrigin( ).cross(stateTwistRot).cross(stateTwistRot);
