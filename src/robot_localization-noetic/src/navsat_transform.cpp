  ...
  tf_listener_(tf_buffer_)
{
  ...
  if (use_manual_datum_ && nh_priv.hasParam("datum"))
  {
    ...
    try
    {
      ...
      istr >> datum_lat >> datum_lon >> datum_yaw; // 读入基准

      // Try to resolve tf_prefix
      std::string tf_prefix      = "";
      std::string tf_prefix_path = "";
      if (nh_priv.searchParam("tf_prefix", tf_prefix_path)) // 查找参数树中是否存在该参数并返回该参数在参数树中的路径
      {
...
void NavSatTransform::computeTransform( )
{
  ...
  if (!transform_good_ && has_transform_odom_ && has_transform_gps_ && has_transform_imu_)
  {
     ...
     */
    // ENU: east-north-up东北天坐标系
    /* 1. 坐标系不一致和磁偏角
       2. 包括标定数据不准确等因素带来的偏差(例如IMU的角度发生了偏移导致标定数据不再准确)
       3. utm坐标轴和真实的东向和北向不一致
    */
                                                  ...
                                                  const ros::Time &transform_time)
{
  ...
  if (can_transform)
  {
    ...
    // ??? I think author assume that msg of GPS doesn't contain oriention. If msg of GPS contains valid oriention,
  ...
  {
    if (gps_frame_id_ != "") // if gpsframe exists.
    {
...
void NavSatTransform::gpsFixCallback(const sensor_msgs::NavSatFixConstPtr &msg)
{
  ...
  bool good_gps = (msg->status.status != sensor_msgs::NavSatStatus::STATUS_NO_FIX && !std::isnan(msg->altitude) && !std::isnan(msg->latitude) && !std::isnan(msg->longitude));
...
void NavSatTransform::imuCallback(const sensor_msgs::ImuConstPtr &msg)
{
  ...
  if (has_transform_odom_)
  {
    ...
    if (can_transform) // If transform between frames is valid.
    {
